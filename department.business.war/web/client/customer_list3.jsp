...
<script type="text/javascript">
	...
	var customterMenu=[[
	<%boolean flag=false;
	if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_followme_rename")){
		flag=true;%>
	...
	<%}%>
	<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_followme_customerCategory_delete")){
		if(flag){
			%>,<%}%>
 	{
		text:"删除",
 	...
 	<%}%>
 	   ...
 	   <%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_followme_tag_add")){%>
 	   {
		text:"添加标签",
	...
	<%}%>
	var customterLabel=[[
	<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_followme_customerLabel_edit")){%>
 	{
		text:"重命名",
		classname:"smarty_menu_ico1",
		func:function(){
	...
	<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_followme_customerLabel_delete")){%>
	 	   {
			text:"删除",
			classname:"smarty_menu_ico2",
			func:function(){
		...
		$("#list").jqGrid({
			...
			gridComplete: function(){
				...
				for(var i = 0 ; i < ids.length; i++){
					...
					if(row.userId==''){
						<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_followme_configAccount")){%>
						content += "<img src=\"core/common/images/users.png\" width=\"14\" height=\"14\" title=\"设置账号\" onclick=\"createUser('"+id+"');\"  /> ";
						<%}%>
					}else{
						<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_followme_updatePassword")){%>
						content += "<img src=\"core/common/images/users.png\" width=\"14\" height=\"14\" title=\"修改密码\" onclick=\"updateAccountPassword('"+row.userId+"');\"  /> ";
						<%}%>
					}
					<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_followme_view")){%>
					content += "<img src=\"core/common/images/viewbtn.gif\" width=\"14\" height=\"14\" title=\"查看\" onclick=\"viewById('"+id+"');\"  /> "; 
					<%}%>
					<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_followme_edit")){%>
					content += "<img src=\"core/common/images/edit.gif\" width=\"14\" height=\"14\" title=\"编辑\" onclick=\"editById('"+id+"');\"  /> "; 
					<%}%>
					if(row.investmentId!=''){
					<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_followme_investmentView")){%>
	...
	function search(filters){
		$("#filters").val(filters);
		$("#list").jqGrid('setGridParam',{url:"<%=basePath%>customer!list.action?customerSource=followMe",page:1,postData:{filters:filters}}).trigger('reloadGrid');
	}
...
<div class="emailtop">
	<div class="leftemail">
		<ul>
		<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_followme_add")){ %>
		...
		<%} %>
		<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_followme_delete")){ %>
			<li onmouseover="this.className='libg'" onmouseout="this.className=''" onclick="deleteSelected()"><span><img src="core/common/images/emaildel.png"/></span>删除</li>
		<%} %>
		<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_followme_export")){ %>	
		...
		<%} %>
		<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_followme_importCard")){ %>
		...
		<tr>
			<td width="182" valign="top">
				<div class="agency" id="resizable" >
					...
					<div class="agencybtn">
						<ul>
						<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_followme_enterprise_classify")){ %>
				...
				<div class="pm_msglist" id="msglist">
					...
					<div class="searchdivkf">
						...
						<table width="100%" height="25" border="0" cellpadding="0" cellspacing="0">
							<tr>
								<td width="65" title="按名称、编号、引进人、跟踪人等关键字查询 我跟踪的客户里">关键字： </td>
								<td width="105">
