...
<script type="text/javascript">
	...
	var customterMenu=[[
	<%boolean flag=false;
	if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_importme_rename")){
		flag=true;%>
	...
	<%}%>
	<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_importme_customerCategory_delete")){
		if(flag){
			%>,<%}%>
 	{
		text:"删除",
 	...
 	<%}%>
 	   ...
 	   <%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_importme_tag_add")){%>
 	   {
		text:"添加标签",
	...
	<%}%>
	var customterLabel=[[
	<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_importme_customerLabel_edit")){%>
 	{
		text:"重命名",
		classname:"smarty_menu_ico1",
		func:function(){
	...
	<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_importme_customerLabel_delete")){%>
	 	   {
			text:"删除",
			classname:"smarty_menu_ico2",
			func:function(){
	...
	function initList(){
		...
		$("#list").jqGrid({
	    	url:'<%=basePath%>customer!list.action?type=${param.type}&&incubated=${param.incubated}&&statusType=${param.status}&&improveArchives=${param.improveArchive}&&customerSource=${param.customerSource}',
			colModel: [
				{label:'企业名称', name:'name', align:"center",width:180,formatter:customerName}, 
			    ...
			    {label:'引进人', name:'importName', align:"center",width:60},
			    {label:'跟踪人', name:'hostName', align:"center",width:60},
			    ...
			    {label:'联系电话', name:'customerInfo.officePhone', align:"center", hidden:true},
			    ...
			    {label:'孵化状态', name:'incubationInfo.statusName', align:"center",width:60},
			...
			gridComplete: function(){
				...
				for(var i = 0 ; i < ids.length; i++){
					...
					if(row.userId==''){
						<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_importme_configAccount")){%>
						content += "<img src=\"core/common/images/users.png\" width=\"14\" height=\"14\" title=\"设置账号\" onclick=\"createUser('"+id+"');\"  /> ";
						<%}%>
					}else{
						<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_importme_updatePassword")){%>
						content += "<img src=\"core/common/images/users.png\" width=\"14\" height=\"14\" title=\"修改密码\" onclick=\"updateAccountPassword('"+row.userId+"');\"  /> ";
						<%}%>
					}
					<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_importme_view")){%>
					content += "<img src=\"core/common/images/viewbtn.gif\" width=\"14\" height=\"14\" title=\"查看\" onclick=\"viewById('"+id+"');\"  /> "; 
					<%}%>
					<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_importme_edit")){%>
					content += "<img src=\"core/common/images/edit.gif\" width=\"14\" height=\"14\" title=\"编辑\" onclick=\"editById('"+id+"');\"  /> "; 
					<%}%>
					if(row.investmentId!=''){
					<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_importme_investmentView")){%>
	...
	function viewById(id){
		fbStart($($("#list").getRowData(id).name).text(),'<%=basePath %>customer!view.action?id='+id,870,460);
	}
	function viewInvestmentById(investmentById){
		fbStart('项目信息','<%=basePath %>investment!view.action?id='+investmentById,700,364);
	}
	function editById(id){
		fbStart('编辑企业档案','<%=basePath %>customer!edit.action?id='+id,700,550);
	}
	function reloadList(){
	...
	function search(filters){
		$("#filters").val(filters);
		$("#list").jqGrid('setGridParam',{url:"<%=basePath%>customer!list.action?customerSource=importMe",page:1,postData:{filters:filters}}).trigger('reloadGrid');
	}
...
<div class="emailtop">
	<div class="leftemail">
		<ul>
		<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_importme_add")){ %>
			<li onmouseover="this.className='libg'" onmouseout="this.className=''" onclick="fbStart('新建企业档案','<%=basePath %>customer!add.action?type=${param.type }&&form=index',700,519);"><span><img src="core/common/images/emailadd.gif" width="15" height="13" /></span>新建</li>
		<%} %>
		<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_importme_delete")){ %>
			<li onmouseover="this.className='libg'" onmouseout="this.className=''" onclick="deleteSelected()"><span><img src="core/common/images/emaildel.png"/></span>删除</li>
		<%} %>
		<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_importme_export")){ %>	
		...
		<%} %>
		<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_importme_importCard")){ %>
		...
		<tr>
			<td width="182" valign="top">
				<div class="agency" id="resizable" >
					...
					<div class="agencybtn">
						<ul>
						<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_importme_enterprise_classify")){ %>
				...
				<div class="pm_msglist" id="msglist">
					...
					<div class="searchdivkf">
						...
						<table width="100%" height="25" border="0" cellpadding="0" cellspacing="0">
							<tr>
								<td width="65" title="按名称、编号、引进人、跟踪人等关键字查询  我引进的客户里">关键字： </td>
								<td width="105">
									<table width="100%" border="0" cellspacing="2" cellpadding="0">
										<tr>
											<td width="180">
												<search:input id="name" dataType="string" field="infoAll" op="cn" inputClass="inputauto"/>
											</td>
						  				</tr>
									</table>
								</td>
