...
<title>无标题文档</title>
	...
	<script type="text/javascript">
		...
		function appendRoom(obj){
			...
			var a = $("<a></a>",{href:"javascript:void(0)",click:function(){deleteRoom($(this).parent().parent(),1);}}).append($("<img src='core/common/images/del.gif' title='删除'/>"));
			var op = $("<td></td>").addClass("centertd").append(a);
		...
		function countDays(yearNum,yearNow,startDateC,endDateC){
			var d=yearNow+'-'+'1'+'-'+'1';
			var dd=parseDate(startDateC); 
			var date=new Date(dd);
			date.setFullYear(date.getFullYear()+yearNum);
			date.setDate(date.getDate()-1);
			var newDate=(date.getFullYear()+"-"+(date.getMonth()+1)+"-"+date.getDate());
			var beforeDay=DateDiff(d,newDate)+1;//无增长率天数
			var afterDay=DateDiff(newDate,endDateC);//有增长率天数
		...
		function addBillRent(){
			...
			fbStart('添加资金计划','<%=basePath %>contract!addBillRent.action?roomIds = '+roomIds+'&startDate='+$('#startDate').val()+'&endDate='+$('#endDate').val(),500,268);
		}
		...
		function addByRating(){
		    ...
		    if(y==''||r=='') return;
				  $("#noData").remove();
					...
					if(flag){
						$("#addRating").children().eq(i).before(html);
						flag=false;
...
<div class="basediv" style="margin-top:0px; height:480px; overflow:hidden; margin-top:3px;">
<div id="scrollDiv" style=" overflow-x:hidden;  height:480px; overflow-y:scroll; position:relative;">
<div class="basediv">
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" >
		<tr>
			<td class="layertdleft100"><span class="psred">*</span>合同编号：</td>
      		...
      		<td class="layertdleft100">物业名称：</td>
      		<td class="layerright" width="25%"><input name="contract.propertyName" type="text" class="inputauto" style="width:99%;"  /></td>
    	</tr>
    	<tr>
	      	<td class="layertdleft100">房屋产权证号：</td>
	      	<td class="layerright" >
				<dd:select name="contract.propertyNoId" key="business.0036" />
		    </td>
	      	<td class="layertdleft100">座落位置：</td>
	      	<td class="layerright" width="33%"><input name="contract.address" type="text" class="inputauto" style="width:99%;"  /></td>
      	</tr>
      	<tr>
...
-->         	
      	...
      	<tr style="display:none">
      		...
      		<td class="layerright" ><input id="areaAmount" name="contract.overallFloorage" type="text" class="inputauto" style="width:55%;" /> 平方米</td>
	      	<td class="layertdleft100">合同总金额：</td>
	      	<td class="layerright" ><input id="payAmount" name="contract.totalAmount" type="text" class="inputauto" style="width:55%;" /> 元</td>
      		
      	</tr>
      	<tr>
	      	<!-- <td class="layertdleft100">待交付日期：</td>
	      	<td class="layerright"><table width="143" border="0" cellspacing="0" cellpadding="0">
	        	<tbody>
		          <tr>
      		...
      		<td class="layerright" >
      			<input onkeyup="if(isNaN(value))execCommand('undo')" onafterpaste="if(isNaN(value))execCommand('undo')" type="text" id="realFeeAmount" name="contract.realFeeZJ" class="inputauto"/>
      		</td>
			...
			<tbody >
				 <tr>
		            ...
		            <td width="300" class="tdcenter" onmousemove="this.className='tdcenterover'" onmouseout="this.className='tdcenter'">增长率&nbsp;(%)</td>
	          		 <td width="108" class="tdcenter" onmousemove="this.className='tdcenterover'" onmouseout="this.className='tdcenter'">操作</td>	
	          	</tr>
	          	</tbody>
			<tbody id="addRating">
	          	<tr id="noData">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="nowrap">
			<tbody>
				<tr>
		            ...
		            <td width="130" class="tdcenter" onmousemove="this.className='tdcenterover'" onmouseout="this.className='tdcenter'">单价 </td>
		            <td width="110" class="tdcenter" onmousemove="this.className='tdcenterover'" onmouseout="this.className='tdcenter'">租用面积 </td>
		            <td width="165" class="tdcenter" onmousemove="this.className='tdcenterover'" onmouseout="this.className='tdcenter'">租用日期 </td>
		            <td width="70" class="tdrightc" onmousemove="this.className='tdcenterover'" onmouseout="this.className='tdcenter'">操作</td>
	          	</tr>
	         </tbody>
  			...
  			<tbody>
    			<tr>
      				...
      				<td colspan="4" class="layerright">
                    	<textarea name="contract.policy" style="height:60px;resize:none;" class="inputauto"></textarea>
                    </td>
    			</tr>
  			</tbody>
  		</table>
