...
<script type="text/javascript">
	...
	function initList(){
		...
		$("#list").jqGrid({
	    	url:'<%=basePath%>contract!list.action?type=${param.type}&&contractSource=${param.contractSource}',
			colModel: [
			...
			gridComplete: function(){
				...
				for(var i = 0 ; i < ids.length; i++){
					var id = ids[i];
					var content = "";
				<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_view")){ %>
			...
			if($(this).getCell(id,"state.title")!='已关闭'){
				...
				if($(this).getCell(id,"state.title")!='已执行'){
				<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_edit")){ %>
		...
		<ul>
			<li onmouseover="this.className='libg'" onmouseout="this.className=''" onclick="$('#meterlayer').toggle();" style="position:relative; z-index:99999;"><span><img src="core/common/images/emailadd.gif" width="15" height="13" /></span>新建<em class="em"><img src="core/common/images/arrow.png" /></em>					
				<div class="meterlayer" id="meterlayer" style="display:none; position:absolute; left:5px;" >
					<dl>
					<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_addCustomer")){ %>
					...
					<%} %>
					<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_addFactory")){ %>
					...
					<%} %>
					<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_addOffice")){ %>
