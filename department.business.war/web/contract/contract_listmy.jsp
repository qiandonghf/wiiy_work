...
<script type="text/javascript">
	...
	function initList(){
		...
		$("#list").jqGrid({
	    	url:'<%=basePath%>contract!list.action?contractSource=${param.contractSource}',
			colModel: [
				{label:'ID', name:'id', align:"center",hidden:true}, 
				{label:'合同编号', name:'code', align:"center",width:90}, 
				{label:'企业名称', name:'customerName',index:'customer', align:"center"}, 
				{label:'合同类型', name:'item.title',index:'item', align:"center",width:100}, 
				{label:'合同名称', name:'name', align:"center",width:220,hidden:true}, 
				{label:'负责人', name:'manager.realName', align:"center",width:90}, 
				{label:'合同金额', name:'totalAmount',width:80,formatter:'number',formatoptions:{decimalSeparator:".", thousandsSeparator: " ", decimalPlaces: 2, defaulValue: 0},align:"center"}, 
			    {label:'合同录入日期', name:'createTime', align:"center",width:80,formatter:'date',formatoptions:{srcformat: 'Y-m-d', newformat: 'Y-m-d'}},
			    {label:'合同到期日期', name:'endDate', align:"center",width:80,formatter:'date',formatoptions:{srcformat: 'Y-m-d', newformat: 'Y-m-d'}},
			    {label:'合同开始日期', name:'startDate',hidden:true,width:80, align:"center",formatter:'date',formatoptions:{srcformat: 'Y-m-d', newformat: 'Y-m-d'},hidden:true},
			...
			gridComplete: function(){
				...
				for(var i = 0 ; i < ids.length; i++){
					var id = ids[i];
					var content = "";
				<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_view_my")){ %>
					content += "<img src=\"core/common/images/viewbtn.gif\" width=\"14\" height=\"14\" title=\"查看\" onclick=\"viewById('"+id+"');\"  /> "; 
				<%}%>
			if($(this).getCell(id,"state.title")!='已关闭'){
				<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contractReview_add")){ %>
				...
				<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contractExpireApproval_add")){ %>
					content += "<img src=\"core/common/images/application_add.png\" width=\"14\" height=\"14\" title=\"发起到期合同审批会签\" onclick=\"expire('"+id+"');\"  /> "; 
				<%}%>
				}
				if($(this).getCell(id,"state.title")!='已执行'){
				<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_edit_my")){ %>
					content += "<img src=\"core/common/images/edit.gif\" width=\"14\" height=\"14\" title=\"编辑\" onclick=\"editById('"+id+"');\"  /> "; 
				<%}%>
				}
			}
				<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_delete_my")){ %>
	...
	function viewById(id){
		fbStart('查看合同','<%=basePath %>contract!view.action?id='+id,765,519);
	}
	function editById(id){
		fbStart('编辑合同','<%=basePath %>contract!edit.action?id='+id,750,503);
	}
		...
		<ul>
			<li onmouseover="this.className='libg'" onmouseout="this.className=''" onclick="$('#meterlayer').toggle();" style="position:relative; z-index:99999;"><span><img src="core/common/images/emailadd.gif" width="15" height="13" /></span>新建<em class="em"><img src="core/common/images/arrow.png" /></em>					
				<div class="meterlayer" id="meterlayer" style="display:none; position:absolute; left:5px;" >
					<dl>
					<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_addCustomer_my")){ %>
						<dt><a href="javascript:void(0)" onclick="fbStart('<%=ContractItemEnum.DSZLHT.getTitle() %>','<%=basePath %>web/contract/contract_add_customer_rent_1.jsp?type=<%=ContractItemEnum.DSZLHT.name() %>',750,520);"><%=ContractItemEnum.DSZLHT.getTitle() %></a></dt>
					<%} %>
					<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_addFactory_my")){ %>
						<dt><a href="javascript:void(0)" onclick="fbStart('<%=ContractItemEnum.CFZLHT.getTitle() %>','<%=basePath %>web/contract/contract_add_customer_rent_2.jsp?type=<%=ContractItemEnum.CFZLHT.name() %>',750,540);"><%=ContractItemEnum.CFZLHT.getTitle() %></a></dt>
					<%} %>
					<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_addOffice_my")){ %>
						<dt><a href="javascript:void(0)" onclick="fbStart('<%=ContractItemEnum.FHXY.getTitle() %>','<%=basePath %>web/contract/contract_add_customer_rent_3.jsp?type=<%=ContractItemEnum.FHXY.name() %>',750,520);"><%=ContractItemEnum.FHXY.getTitle() %></a></dt>
					<%} %>
					</dl>
				</div>
			</li>
			...
			<%} %>
			<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_contract_export_my")){ %>
...
<div class="searchdiv">
	<table width="100%" height="25" border="0" cellpadding="0" cellspacing="0">
		<tr>
			<td width="70"title="按合同名称、编号、负责 人等关键字查询">关键字：  </td>
			<td width="150">
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
						<td width="150"><search:input dataType="string" field="infoAll" op="cn" inputClass="inputauto"/></td>
					</tr>
				</table>
			</td>
