...
<script>
	...
	function initList(){
		...
		$("#list").jqGrid({
	    	url:'<%=BaseAction.rootLocation %>/common/billRemind!list.action?feeType=${param.feeType}',
			colModel: [
			...
			gridComplete: function(){
				...
				for(var i = 0 ; i < ids.length; i++){
					...
					<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_businessBillRemind_view")){%>
						content += "<a href=\"javascript:void(0)\" onclick=\"viewById('"+id+"');\">查看</a> ";
					<%}%>
					<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_businessBillRemind_remind")){%>
						content += "<a href=\"javascript:void(0)\" onclick=\"remindById('"+id+"');\">催缴</a> ";
					<%}%>
					<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_businessBillRemind_delete")){%>
	...
	function remindById(id){
		if(confirm("确定设置已催缴")){
			$.post("<%=basePath%>businessBillRemind!remind.action?id="+id,function(data){
	...
	function viewById(id){
		fbStart($("#list").getRowData(id)["customer.name"],'<%=basePath %>businessBillRemind!view.action?id='+id,550,360);
	}
	function reloadList(){
		$("#list").trigger("reloadGrid");
	}
	function deleteById(id){
		if(confirm("确定要删除吗")){
			$.post("<%=basePath%>businessBillRemind!delete.action?id="+id,function(data){
				showTip(data.result.msg,2000);
	        	if(data.result.success){
	        		$("#list").trigger("reloadGrid");
	        	}
	...
	function deleteSelected(){
		...
		if(ids!='' && confirm("确定要删吗")){
			$.post("<%=basePath%>businessBillRemind!delete.action?ids="+ids,function(data){
				showTip(data.result.msg,2000);
	        	if(data.result.success){
	        		$("#list").trigger("reloadGrid");
	        	}
...
<body>
<form action="<%=basePath%>businessBillRemind!export.action" id="exportForm" method="post">
	...
	<map name="Map" id="Map">
		...
		<area shape="rect" coords="54,0,73,19" href="javascript:void(0)" onclick="fbSearch('高级搜索','<%=basePath %>web/businessBillRemind/businessBillRemind_search.jsp',550,410);" />
	</map>
</body>
</html>
