...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javascript">
			...
			var baseMenu = 
			<%
				boolean baseMenu = BusinessActivator.getHttpSessionService().isInResourceMap("business_project_allView") 
					|| BusinessActivator.getHttpSessionService().isInResourceMap("business_project_allEdit") 
					|| BusinessActivator.getHttpSessionService().isInResourceMap("business_project_allPrint") 
					|| BusinessActivator.getHttpSessionService().isInResourceMap("business_project_allImportant") 
					|| BusinessActivator.getHttpSessionService().isInResourceMap("business_project_allSleep") 
					|| BusinessActivator.getHttpSessionService().isInResourceMap("business_project_allSubmission") 
					|| BusinessActivator.getHttpSessionService().isInResourceMap("business_project_allDelete")
					||BusinessActivator.getHttpSessionService().isInResourceMap("business_countersign_begin")
					||BusinessActivator.getHttpSessionService().isInResourceMap("business_project_allStatus");
			if(baseMenu) {%> 	
				[
			<%if(BusinessActivator.getHttpSessionService().isInResourceMap("business_project_allView")){
				flag = true;
			%>    
		    {
				text: "打开",
		    ...
		    <%}%>
			<%
				if(BusinessActivator.getHttpSessionService().isInResourceMap("business_project_allEdit")){
					if(flag){
						%>,<%
					}
					flag = true;
		    ...
		    <%}%>
			<%
				if(BusinessActivator.getHttpSessionService().isInResourceMap("business_project_allPrint")){
					if(flag){
			%>,<%}flag = true;%>
		    {
				text: "<%=resourceMap.get("business_project_allPrint").getName() %>",
			...
			var importMenu = 
			<%
			boolean importMenu = BusinessActivator.getHttpSessionService().isInResourceMap("business_project_allImportant");
			if(importMenu){%> 	
				[{
					text: "重要性",
			...
			var setUpMenu = <% flag = false;
				boolean business_project_newStatus = BusinessActivator.getHttpSessionService().isInResourceMap("business_project_allStatus");
		   ...
		   var deleteMenu = 
			   <%
			   	boolean delete = BusinessActivator.getHttpSessionService().isInResourceMap("business_project_allDelete");
			   	if(delete){%>  
		          [{
					text: "删除",
		     ...
		     <%}else{%>""<%}%>;
			     ...
			     var submitMenu =
			     <%
			     	boolean submit = BusinessActivator.getHttpSessionService().isInResourceMap("business_project_allSubmitEnter");
			     	if(submit){%> 
					  [{
						text: "提交入驻",
		...
		<div class="emailtop">
			<div class="leftemail">
				<ul>
					...
					<li onmouseover="this.className='libg'" onmouseout="this.className=''" onclick="fbStart('新建申请单','<%=basePath %>web/investment/investment_add.jsp',720,538);"><span><img src="core/common/images/emailadd.gif" /></span>新建</li>
				</ul>
			</div>
		</div>
		<div id="container">
						...
						<div class="write_list" style="border-right:1px solid #ddd; width:355px;" id="resizable">
							...
							<table width="100%" border="0" cellspacing="0" cellpadding="0" >
								<tr>
									...
									<td class="tdleftc" width="80">更新时间</td>
								</tr>
							</table>
							<div id="contentDiv" style="height:300px;overflow-y:auto;" >
								<table width="100%" border="0" cellspacing="0" cellpadding="0" id="lsittable">
									...
									<tr onmouseout="this.style.background='#fff';keepHighlight(this)" onmouseover="this.style.background='#f4f4f4';"  class="investmenttr${investment.investmentStatus }" style="cursor: pointer;" onclick="openRight(${investment.id});highlight(this);">
										...
										<td class="centertd" style="width:80px"><fmt:formatDate value="${investment.modifyTime}" pattern="yyyy-MM-dd"/></td>
									</tr>
									</c:forEach>
								</table>
							</div>
