	...
	<script type="text/javascript">
		...
		function initList(){
			var height = $(this).height()-60;
 			$("#list").jqGrid({
 		    	url:'<%=BaseAction.rootLocation%>/parkmanager.pf/bill!list.action?department=ENGINEERING&contractId=${param.id }',
 				colModel: [
 					...
 					{label:'流程实体Id',width:40,name:'processInstanceId',align:"center",hidden:true}, 
 				...
 				gridComplete: function(){
 					...
 					for(var i = 0 ; i < ids.length; i++){
 						var id = ids[i];
 						var content = "";
 						var pid = $(this).getCell(id,'processInstanceId');
 						...
 						<%if(view){%>
 						content += "<img src=\"core/common/images/viewbtn.gif\" width=\"14\" height=\"14\" title=\"查看\" onclick=\"billView('"+id+"','"+pid+"');\"  /> ";
 						<%}%>
 						<%-- <%if(edit){%>
 		...
 		function billView(id,pid){
			var src = "<%=BaseAction.rootLocation%>/parkmanager.pf/web/contact/bill/bill_launch_my.jsp?pid="+pid+"&id="+id;
 			fbStart('查看结算流程',src,650,489);
 		}
