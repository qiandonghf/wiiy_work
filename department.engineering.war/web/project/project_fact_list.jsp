...
<script type="text/javascript">
		...
		$("#"+list).jqGrid({
			...
			gridComplete: function(){
				...
				for(var i = 0 ; i < ids.length; i++){
					var id = ids[i];
					var content = "";
					<%boolean add = EngineeringActivator.getHttpSessionService().isInResourceMap("department_engineering_project_fact_add");
					boolean edit = EngineeringActivator.getHttpSessionService().isInResourceMap("department_engineering_project_fact_edit");
					boolean view = EngineeringActivator.getHttpSessionService().isInResourceMap("department_engineering_project_fact_view");
					boolean del = EngineeringActivator.getHttpSessionService().isInResourceMap("department_engineering_project_fact_delete");%>
					<%if(view){%>
	...
	function viewById(id){
		fbStart('查看实际进度','<%=basePath%>fact!view.action?id='+id,650,211);
	}
	function addById(id){
		fbStart('新建实际进度','<%=basePath %>fact!add.action?id='+id,650,242);
	}
	function editById(id){
		fbStart('编辑实际进度','<%=basePath%>fact!edit.action?id='+id,650,242);
	}
	function deleteById(id){
		if(confirm("您确定要删除")){
			$.post("<%=basePath%>fact!delete.action?id="+id,function(data){
	...
	function deleteSelected(){
		...
		if(ids==""){
			...
			$.post("<%=basePath%>fact!delete.action?ids="+ids,function(data){
