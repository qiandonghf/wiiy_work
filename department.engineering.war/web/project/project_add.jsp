...
<title>无标题文档</title>
	...
	<script type="text/javascript">
		...
		function changeState(obj){
			...
			if($(obj).attr("name") == 'engineering.finished'){
				var p = $(obj).parent();
				var c = $(p).find("span");
				if($(obj).val() == 'NO')$(c).text("未完成");
				else $(c).text("已完成");
			}else if($(obj).attr("name") == 'engineering.audit'){
				var p = $(obj).parent();
				var c = $(p).find("span");
		...
		function initForm(){
			$("#form1").validate({
				rules: {
					...
					"engineering.schedules":"required",
				...
				messages: {
					...
					"engineering.schedules":"请输入项目进度数",
					...
					$(form).ajaxSubmit({
				        ...
				        success: function(data){
				        	...
				        	if(data.result.success){
				        		if("${param.from}" == 'index')
				        			setTimeout("getOpener().reloadList('project');parent.fb.end()", 2000);
				        		else
...
<div class="divlays" style="margin:0px;">
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
		<tr>
			...
			<td class="layerright"><input name="engineering.name" type="text" class="inputauto" value="" /></td>
       		<td class="layertdleft100"><span class="psred">*</span>编号：</td>
			<td class="layerright">
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
						<td width="100"><input id="code" name="engineering.code" type="text" class="inputauto" /></td>
       	...
       	<tr>
		    ...
		    <td class="layerright"><input name="engineering.abbreviation" type="text" class="inputauto" value="" /></td>
			<td class="layertdleft100"><span class="psred">*</span>项目进度数：</td>
		    <td class="layerright"><input name="engineering.schedules" onkeyup="checkDouble(this);" class="inputauto"/></td>
       	</tr>
       	<tr>
			...
			<td class="layerright">
				<table width="143" border="0" cellspacing="0" cellpadding="0">
			        <tr>
			          <td>
			          	<input id="supplierId" name="engineering.supplierId" type="hidden"/>
			          	<input id="supplier" name="engineering.supplierName" class="inputauto" onclick="selectCustomer('supplier')" readonly="readonly"/></td>
			...
			<td class="layerright">
				<table width="143" border="0" cellspacing="0" cellpadding="0">
			        <tr>
			          <td>
				          <input id="customerId" name="engineering.customerId" type="hidden"/>
				          <input id="customer" name="engineering.customerName" class="inputauto" onclick="selectCustomer('customer')" readonly="readonly"/>
				      </td>
       	...
       	<tr>
			...
			<td class="layerright" >
				<enum:select name="engineering.status" type="com.wiiy.common.preferences.enums.ProjectStatusEnum" />
		    </td>
		    <td class="layertdleft100">重要程度：</td>
			<td class="layerright">
				<enum:select name="engineering.priority" type="com.wiiy.common.preferences.enums.ProjectPriorityEnum" />
		    </td>
    	</tr>
    	<tr>
			...
			<td class="layerright">
				<table border="0" cellspacing="0" cellpadding="0">
			        <tr>
			          <td>
			          	<input id="managerId" name="engineering.managerId" type="hidden" />
			          	<input id="manager" class="inputauto" onclick="selectUser('manager')" readonly="readonly" />
			          </td>
			...
			<td class="layerright" >
				<table border="0" cellspacing="0" cellpadding="0">
			        <tr>
			          <td>
			          	<input id="handlingId" name="engineering.handlingId" type="hidden" />
       	...
       	<tr>
       		...
       		<td class="layerright" >
	       		<dd:select name="engineering.currencyTypeId" key="project.0001"/>
       		</td>
       		<td class="layertdleft100">金额：</td>
	      	<td class="layerright"><input name="engineering.amount" type="text" value="0.00" class="inputauto"  /></td>
       	</tr>
       	<tr>
    		<td class="layertdleft100">预计花费：</td>
	      	<td class="layerright"><input name="engineering.expectedCost" type="text" value="0.00" class="inputauto"/></td>
    		<td class="layertdleft100">实际花费：</td>
    		<td class="layerright" ><input name="engineering.actualCost" type="text" value="0.00" class="inputauto"/></td>
    	</tr>
    	<tr>
    		<td class="layertdleft100">收付方式：</td>
       		<td class="layerright">
				<enum:select name="engineering.payment" type="com.wiiy.common.preferences.enums.PaymentTypeEnum"/>
       		</td>
       		<td class="layertdleft100">结算方式：</td>
       		<td class="layerright" >
				<enum:select name="engineering.settlement" type="com.wiiy.common.preferences.enums.SettlementTypeEnum" />
       		</td>
    	</tr>
    	<tr>
     		...
     		<td class="layerright">
	     		<table border="0" cellspacing="0" cellpadding="0">
			        <tr>
			          <td><input id="startDate" name="engineering.startDate" type="text" readonly="readonly" class="inputauto" onclick="showCalendar('startDate');"/></td>
		    ...
		    <td class="layertdleft100"><span class="psred">*</span>项目结束日期：</td>
     		<td class="layerright">
	     		<table border="0" cellspacing="0" cellpadding="0">
			        <tr>
			          <td><input id="endDate" name="engineering.endDate" type="text" readonly="readonly" class="inputauto" onclick="showCalendar('endDate')"/></td>
    	...
    	<tr>
    		<td class="layertdleft100"><span class="psred">*</span>项目签订日期：</td>
	      	<td class="layerright">
		      	<table border="0" cellspacing="0" cellpadding="0">
		          <tr>
		            <td><input id="signDate" name="engineering.signDate" readonly="readonly" type="text" class="inputauto" onclick="showCalendar('signDate')"/></td>
	      	...
	      	<td class="layerright" colspan="2">
				<label><input name="engineering.audit" type="checkbox" onclick="changeState(this);" value="NO" /><span>未审核</span></label>
				<label><input name="engineering.finished" type="checkbox" onclick="changeState(this);" value="NO" /><span>未完成</span></label>
		   		<label><input name="engineering.published" type="checkbox" onclick="changeState(this);" value="NO" />公开标志</label>
	      	</td>
       	</tr>
       	<tr>
	    	<td class="layertdleft100">项目简介：</td>
	    	<td colspan="3" class="layerright" style="padding-bottom:3px;">
	    		<textarea name="engineering.introduction" class="inputauto" style="resize:none;height:40px;"></textarea>
	    	</td>
	    </tr>
       	<tr>
       		<td class="layertdleft100">备注信息：</td>
			<td class="layerright"  colspan="3">
	    		<textarea name="engineering.memo" class="inputauto" style="resize:none;height:40px;"></textarea>
			</td>
       	</tr>
  </table>
  </div>
