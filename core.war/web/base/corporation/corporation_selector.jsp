...
<script type="text/javascript">
	...
	function initList(){
		$("#list").jqGrid({
	    	url:'<%=basePath%>corporation!selectList.action',
			datatype: 'json',
			mtype: 'POST',
		...
		if('import'==type){
			...
			$(ids).each(function (index, id) {
				if ($("#list").find('#'+id+' input').prop('checked')) {
					fb.end();
					getOpener().addCustomer(id);
				}
			});
		}else{
			try{
				if(typeof(getOpener().eval("setSelectCorporation")) == "function"){
					var ids = $("#list").jqGrid("getDataIDs");
					$(ids).each(function (index, id) {
						if ($("#list").find('#'+id+' input').prop('checked')) {
							var rowData = $("#list").jqGrid("getRowData", id);
							getOpener().setSelectCorporation(rowData);
							return;
						}
					});
				}
			...
			try{
				if(typeof(getOpener().eval("setSelectCorporations")) == "function"){
					var array = new Array();
					var selectIds = $("#list").getGridParam("selarrrow");
					for(var i = 0 ; i < selectIds.length; i++){
						array[i] = $("#list").getRowData(selectIds[i]);
					}
					getOpener().setSelectCorporations(array);
				}
			} catch(e){}
		}
		fb.end();
