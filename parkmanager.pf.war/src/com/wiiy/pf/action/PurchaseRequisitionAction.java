 ...
 * @author my
 */
public class PurchaseRequisitionAction extends JqGridBaseAction<PurchaseRequisition>{
	...
	public PurchaseRequisitionAction() {
	...
	public String save(){
		...
		purchaseRequisition.setUserId(user.getId().toString());
		purchaseRequisition.setApplyTime(new Date());
		result = purchaseRequisitionService.save(purchaseRequisition);
		Map<String, Object> variables = new HashMap<String, Object>();
		String businessKey = purchaseRequisition.getId().toString();
		identityService.setAuthenticatedUserId(user.getUsername());
		ProcessInstance processInstance = runtimeService.
				startProcessInstanceByKey("purchase", businessKey, variables);
		...
		purchaseRequisition.setProcessInstanceId(instanceId);
		purchaseRequisitionService.update(purchaseRequisition).getValue();
		purchaseRequisition = purchaseRequisitionService.getBeanByFilter(
				new Filter(PurchaseRequisition.class).
				eq("processInstanceId", instanceId)).getValue();
		List<ContactBaseAtt> atts = getListFromJSON(filePath);
		for (ContactBaseAtt att : atts) {
			att.setContactId(purchaseRequisition.getId());
			att.setProcessInstanceId(instanceId);
			contactBaseAttService.save(att);
		}
		identityService.setAuthenticatedUserId(null);
		ContactLogUtil.saveLog(purchaseRequisition.getId(), ProcessTypeEnum.BILL, user, taskId);
		return JSON;
	}
	...
	public String view(){
		result = purchaseRequisitionService.getBeanById(id);
		return VIEW;
	}
	...
	public String update(){
       ...
       try {
    		...
    		boolean cwcn = variables.containsKey("cwcnBackReason");
           	if (cwcn) {
           		PurchaseRequisition dbBean = purchaseRequisitionService.getBeanById(purchaseRequisition.getId()).getValue();
           		purchaseRequisitionService.update(dbBean);
			}
           	//发送短信
           	if(sendEmail){
           		try{
	...
	public String delete(){
		...
		if(id!=null){
			result = purchaseRequisitionService.deleteById(id);
		} else if(ids!=null){
			result = purchaseRequisitionService.deleteByIds(ids);
		}
		return JSON;
	}
	
	public String list(){
		return refresh(new Filter(PurchaseRequisition.class));
	}
	
	@Override
	protected List<PurchaseRequisition> getListByFilter(Filter fitler) {
		return purchaseRequisitionService.getListByFilter(fitler).getValue();
	}
