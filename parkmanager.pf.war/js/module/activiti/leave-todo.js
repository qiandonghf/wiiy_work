...
function loadDetail(id, withVars, callback) {
    var dialog = this;
    //alert('加载详细信息loadDetail,id='+id+',withVars='+withVars+',callback='+callback);
    $.getJSON('/parkmanager.pf/contact!detail.action?type=leave&id='+id, function(data) {
        detail = data.result.value;
        $.each(detail, function(k, v) {
			...
			if (k == 'applyTime' || k == 'startTime' || k == 'endTime') {
				...
				$('.view-info td[name=' + k + ']', dialog).text(v.title);
			}else {
...
function loadDetailWithTaskVars(leaveId, taskId, callback) {
    ...
    //alert('加载详细信息，同时读取流程任务变量loadDetailWithTaskVars,leaveId='+leaveId+',taskId='+taskId+',callback='+callback);
    $.getJSON('/parkmanager.pf/contact!loadLeaveWithVars.action?id=' + leaveId + '&taskId=' + taskId, function(data) {
        detail = data.result.value;
        $.each(detail, function(k, v) {
            // 格式化日期
			if (k == 'applyTime' || k == 'startTime' || k == 'endTime') {
...
function complete(taskId, variables) {
    var dialog = this;
    //alert('完成任务complete,taskId='+taskId+',variables='+variables);
	// 转换JSON为字符串
	...
	/*$.blockUI({
        message: '<h2><img src="/activiti/images/ajax/loading.gif" align="absmiddle"/>正在提交请求……</h2>'
    });*/
	
	// 发送任务完成请求
	
    $.post('/parkmanager.pf/contact!complete.action?taskId=' + taskId, {
        keys: keys,
        values: values,
        types: types
    }, function(resp) {
        if (resp.result.msg) {
            showTip('任务完成',2000);
            location.href='/parkmanager.pf/contact!taskList.action';
        } else {
        	showTip('操作失败',2000);
        }
    });
...
var handleOpts = {
	...
	modifyApply: {
		...
		open: function(id, taskId) {
			...
			// 打开对话框的时候读取请假内容
			loadDetailWithTaskVars.call(this, id, taskId, function(d) {
				var data=d.result.value;
				// 显示驳回理由
				$('.info').show().html("<b>部门领导：</b>" + (data.variables.leaderBackReason || "") + "<br/><b>HR：</b>" + (data.variables.hrBackReason || ""));
				
				// 读取原请假信息
				$('#modifyApplyContent #leaveType option[value=' + data.leaveType.title + ']').attr('selected', true);
