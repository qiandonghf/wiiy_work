...
function selectPlan(id){
	...
	var planId = $("#"+clicked+"Id").val();
	var task = "${taskId}";
	var type = task.substr(0,task.indexOf(":"));
	$("input[name='processDefinitionKey']").val(type);
	type = findType(type);
	$("input[name='bill.department']").val(type.toUpperCase());
	var url = '<%=basePath %>web/contact/bill/billPlanRent_selector.jsp?name='+name+"&id="+planId+"&type="+type;
	fbStart("选择结算计划",url,520,361);
} 

function findType(type){
	switch(type){
	case "engineeringBill":
		return "engineering";
	case "synthesisBill":
		return "synthesis";
	}
...
function setSelectedPlan(plan){
	$("#"+clicked).val(plan.code);
	$("#"+clicked+"Id").val(plan.id); 
	/*selectedUser = {id : id, code : code, contractName : contractName , 
						contractId : contractId , contractAmount : contractAmount , 
						planFee : planFee ,	planPayDate : planPayDate};*/
	var planFee = plan.planFee;
	var planPayDate = plan.planPayDate;
	var contractAmount = plan.contractAmount;
	planFee = planFee.indexOf(".") == -1 ?  planFee+".00" : planFee;
	contractAmount = contractAmount.indexOf(".") == -1 ?  contractAmount+".00" : contractAmount;
	planPayDate = planPayDate.substr(0,planPayDate.indexOf("T"));
	$("#planFee").val(planFee); 
	$("#planPayDate").val(planPayDate); 
	$("#contractId").val(plan.contractId); 
	$("#contractName").val(plan.contractName); 
	$("#contractAmount").val(contractAmount); 
	$.ajax({
		 "url"	: 	"<%=basePath%>bill!findCompleteFee.action",
		  type	:	"POST",
		  data	: 	"contractId="+plan.contractId+"&department="+$("input[name='bill.department']").val(),
		  success: function(data){
			  var completedFee = data.billRentDto.completedFee+"";
			  completedFee = completedFee.indexOf(".") == -1 ?  completedFee+".00" : completedFee;
			  $("#completedFee").val(completedFee);
		  }
	});
}
...
<input type="hidden" name="bill.department" value=""/>
         ...
         <div class="basediv" style="min-width:600px;">
			<div class="divlays" style="margin:0px;">
              <table id="insertTxt" width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				<tr>
				    <td class="layertdleft100"><span class="psred">*</span>结算计划：</td>
					<td class="layerright">
						<table width="143" border="0" cellspacing="0" cellpadding="0">
					        <tr>
					          <td>
						          <input id="billId" name="bill.billPlanRentId" type="hidden"/>
						          <input id="bill" name="bill.code" class="inputauto" onclick="selectPlan('bill')" readonly="readonly"/>
						      </td>
					          <td width="21" align="center"><img src="core/common/images/outdiv.gif" width="20" height="22" onclick="selectPlan('bill')" style="cursor:pointer"/></td>
					        </tr>
					    </table>
					</td>
		       		<td class="layertdleft100" >预计结算：</td>
		       		<td class="layerright" >
						<input id="planFee" value="${result.value.planFee }" disabled="disabled" type="text" class="inputauto" />
				    </td>
		       	</tr>
		       	<tr>
		       		<td class="layertdleft100" >预计结算日期：</td>
			      	<td class="layerright" >
						<input id="planPayDate" disabled="disabled" type="text" class="inputauto" />
			      	</td>
				    <td class="layertdleft100">合同名称：</td>
					<td class="layerright" >
						<input id="contractId" name="bill.contractId" type="hidden"/>
						<input id="contractName" disabled="disabled" class="inputauto"/>
				   	</td>
				</tr>
		       	<tr>
		   			<td class="layertdleft100">合同金额：</td>
					<td class="layerright" >
						<input id="contractAmount" disabled="disabled" class="inputauto" />
				   	</td>
				   	<td class="layertdleft100">已结算金额：</td>
					<td class="layerright" >
						<input id="completedFee" disabled="disabled" class="inputauto" />
				   	</td>
				</tr>
				<tr>
					<td class="layertdleft100">流程：</td>
					<td class="layerright" >
						<input disabled="disabled" class="inputauto" value="${billRentDto.taskName }" />
				   	</td>
