...
<script type="text/javascript">
  ...
  var type = "${param.type}";
  name = decodeURI(name);
  var selectedUser={id:"${param.id}", code : name};
  ...
  function showSelectedUser() {
	  $("#selectedUserDiv").text(selectedUser.code);
  }

  function initList(){
	var url = "${contextLocation}department."+type+"/billPlanRent!simpleList.action"
	$("#list").jqGrid({
    	url:url,
		datatype: 'json',
		...
		colModel: [
		    ...
		    {label:'合同Id', name:'contract.id',	hidden:true, index:'name', align:"center"}, 
	...
	function checkSelectedUser() {
		...
		$(ids).each(function (index, id) {
			...
			if($("#list").find('#'+id+' input').prop('checked') == true){
				var obj = $("#list").find('#'+id).children();
				var code = $(obj).eq(2).text();
				var contractId = $(obj).eq(3).text();
				var contractName = $(obj).eq(4).text();
				var contractAmount = $(obj).eq(5).text();
				var planFee = $(obj).eq(6).text();
				var planPayDate = $(obj).eq(7).text();
	...
	function submitSelectedUser() {
		...
		parent.setSelectedPlan(selectedUser);
		parent.fb.end();
	}
</script>
</head>
