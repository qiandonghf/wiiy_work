...
<script type="text/javascript">
	<%if(EstateActivator.getSessionUser(request).getAdmin().equals(BooleanEnum.YES)){%>
	var customterMenu=[[
	<%boolean flag=false;
	if(EstateActivator.getHttpSessionService().isInResourceMap("business_customerCategory_rename")){
		flag=true;%>
	
 	{
		...
		func:function(){
			fbStart('重命名','<%=BaseAction.rootLocation%>/department.business/customerCategory!edit.action?id='+$(this).find('input').val(),300,80);
		}
	}
	<%}%>
	<%if(EstateActivator.getHttpSessionService().isInResourceMap("business_customerCategory_delete")){
		if(flag){
			%>,<%}%>
 	{
		text:"删除",
		...
		func:function(){
			if(confirm("您确定要删除")){
				$.post("<%=BaseAction.rootLocation%>/department.business/customerCategory!delete.action?id="+$(this).find('input').val(),function(data){
 	...
 	<%}%>
 	   ...
 	   <%if(EstateActivator.getHttpSessionService().isInResourceMap("business_tag_add")){%>
 	   {
		text:"添加标签",
		classname:"smarty_menu_ico0",
		func:function(){
			fbStart('添加标签','<%=BaseAction.rootLocation%>/department.business/customerLabel!addByCategoryId.action?id='+$(this).find('input').val(),300,80);
		}
	}
 	   <%}%>]];
	<%}%>
	var customterLabel=[[
	<%if(EstateActivator.getHttpSessionService().isInResourceMap("business_customerLabel_edit")){%>
 	{
		text:"重命名",
		classname:"smarty_menu_ico1",
		func:function(){
			fbStart('重命名','<%=BaseAction.rootLocation%>/department.business/customerLabel!edit.action?id='+$(this).find('input').val(),300,80);
		}
	}
 	<%}%>
 	],[
	<%if(EstateActivator.getHttpSessionService().isInResourceMap("business_customerLabel_delete")){%>
	 	   {
			text:"删除",
			classname:"smarty_menu_ico2",
			func:function(){
				if(confirm("您确定要删除")){
					$.post("<%=BaseAction.rootLocation%>/department.business/customerLabel!delete.action?id="+$(this).find('input').val(),function(data){
						showTip(data.result.msg,1000);
						setTimeout("location.reload()", 1000);
					});
				}
	...
	function loadCustomerLabelByCategoryId(categoryId){
		if($("#customerCategoryLi"+categoryId).hasClass("expandable")){
			$.post("<%=BaseAction.rootLocation%>/department.business/customerLabel!loadCustomerLabelByCategoryId.action?id="+categoryId,function(data){
	...
	function loadMyLabel(categoryId){
		if($("#customerCategoryLi"+categoryId).hasClass("expandable")){
			$.post("<%=BaseAction.rootLocation%>/department.business/customerLabel!loadMyLabel.action",function(data){
				if(data.result.success){
					$("#customerLabelUl"+categoryId).empty();
					for(var i = 0; i < data.result.value.length; i++){
						var customerLabel = data.result.value[i];
	...
	function loadCustomerByLabelId(labelId){
		$("#labelId").val(labelId);
		$("#list").setGridParam({url:'<%=BaseAction.rootLocation%>/department.business/customer!list.action?labelId='+labelId}).trigger('reloadGrid');
	}
	function loadAllCustomer(){
		$("#list").setGridParam({url:'<%=BaseAction.rootLocation%>/department.business/customer!list.action'}).trigger('reloadGrid');
	}
	...
	function showView(name,id){
		fbStart(name,'<%=BaseAction.rootLocation%>/department.business/customer!view.action?id='+id,870,450);
	}
	...
	function initList(){
		...
		$("#list").jqGrid({
	    	url:'<%=BaseAction.rootLocation%>/department.business/customer!newCustomerlist.action',
			colModel: [
			    ...
			    {label:'是否完善档案', name:'improveArchive.title', align:"center",width:60},
			...
			gridComplete: function(){
				...
				for(var i = 0 ; i < ids.length; i++){
					...
					if(row.userId==''){
						<%if(EstateActivator.getHttpSessionService().isInResourceMap("business_configAccount")){%>
						content += "<img src=\"core/common/images/users.png\" width=\"14\" height=\"14\" title=\"设置账号\" onclick=\"createUser('"+id+"');\"  /> ";
						<%}%>
					}else{
						<%if(EstateActivator.getHttpSessionService().isInResourceMap("business_updatePassword")){%>
						content += "<img src=\"core/common/images/users.png\" width=\"14\" height=\"14\" title=\"修改密码\" onclick=\"updateAccountPassword('"+row.userId+"');\"  /> ";
						<%}%>
					}
					<%if(EstateActivator.getHttpSessionService().isInResourceMap("business_customer_view")){%>
					content += "<img src=\"core/common/images/viewbtn.gif\" width=\"14\" height=\"14\" title=\"查看\" onclick=\"viewById('"+id+"');\"  /> "; 
					<%}%>
					<%if(EstateActivator.getHttpSessionService().isInResourceMap("business_customer_edit")){%>
					content += "<img src=\"core/common/images/edit.gif\" width=\"14\" height=\"14\" title=\"编辑\" onclick=\"editById('"+id+"');\"  /> "; 
					<%}%>
					if(row.investmentId!=''){
					<%if(EstateActivator.getHttpSessionService().isInResourceMap("business_customer_investmentView")){%>
						content += "<img src=\"core/common/images/view2.png\" width=\"14\" height=\"14\" title=\"项目信息\" onclick=\"viewInvestmentById('"+row.investmentId+"');\"  /> "; 
					<%}%>
					}
					<%if(EstateActivator.getHttpSessionService().isInResourceMap("business_customer_delete")){%>
	...
	function createUser(id){
		fbStart('设置账号','<%=BaseAction.rootLocation%>/department.business/customer!configAccount.action?id='+id,300,95);
	}
	function updateAccountPassword(userId){
		fbStart('修改密码','<%=BaseAction.rootLocation%>/department.business/customer!updatePassword.action?id='+userId,300,95);
	}
	function viewById(id){
		fbStart($($("#list").getRowData(id).name).text(),'<%=BaseAction.rootLocation%>/department.business/customer!view.action?id='+id,870,460);
	}
	function viewInvestmentById(investmentById){
		fbStart('项目信息','<%=BaseAction.rootLocation%>/department.business/investment!view.action?id='+investmentById,700,364);
	}
	function editById(id){
		fbStart('编辑企业档案','<%=BaseAction.rootLocation%>/department.business/customer!edit.action?id='+id,700,450);
	}
	function reloadList(){
	...
	function deleteById(id){
		if(confirm("确定要删吗")){
			$.post("<%=BaseAction.rootLocation%>/department.business/customer!delete.action?id="+id,function(data){
	...
	function search(filters){
		$("#filters").val(filters);
		$("#list").jqGrid('setGridParam',{url:"<%=BaseAction.rootLocation%>/department.business/customer!list.action",page:1,postData:{filters:filters}}).trigger('reloadGrid');
	}
	function deleteSelected(){
		var ids = $("#list").jqGrid("getGridParam","selarrrow");
		if(ids!='' && confirm("确定要删吗")){
			$.post("<%=BaseAction.rootLocation%>/department.business/customer!delete.action?ids="+ids,function(data){
				showTip(data.result.msg,2000);
	        	if(data.result.success){
	        		$("#list").trigger("reloadGrid");
	        	}
	...
	function initTree(){
		$("#browser").tree({
			animate: true,
			url:"<%=BaseAction.rootLocation%>/department.business/customer!loadCategory.action",
			...
			onBeforeExpand: function(node){
				if(node.id>-1){
					$("#browser").tree("options").url="<%=BaseAction.rootLocation%>/department.business/customer!loadLabelByCategoryId.action";
				} else {
					$("#browser").tree("options").url="<%=BaseAction.rootLocation%>/department.business/customer!loadCategory.action";
				}
			},
	...
	function toImportCard() {
		...
		if(confirm("确定要导入名片夹（客户-企业档案）吗")){
			...
			$.post('<%=BaseAction.rootLocation%>/department.business/customer!importCard.action?ids='+ids,
				function(data){
					showTip(data.result.msg,2000);
				}
			);
...
<body>
<form action="<%=BaseAction.rootLocation%>/department.business/customer!export.action" id="exportForm" method="post">
...
<div class="emailtop">
	<div class="leftemail">
		<ul>
		<%if(EstateActivator.getHttpSessionService().isInResourceMap("business_customer_add")){ %>
			<li onmouseover="this.className='libg'" onmouseout="this.className=''" onclick="fbStart('新建企业档案','<%=BaseAction.rootLocation%>/department.business/customer!add.action?type=${param.type }&&form=index',700,519);"><span><img src="core/common/images/emailadd.gif" width="15" height="13" /></span>新建</li>
		<%} %>
		<%if(EstateActivator.getHttpSessionService().isInResourceMap("business_customer_delete")){ %>
			<li onmouseover="this.className='libg'" onmouseout="this.className=''" onclick="deleteSelected()"><span><img src="core/common/images/emaildel.png"/></span>删除</li>
		<%} %>
		<%if(EstateActivator.getHttpSessionService().isInResourceMap("business_customer_export")){ %>	
		...
		<%} %>
		<%if(EstateActivator.getHttpSessionService().isInResourceMap("business_customer_importCard")){ %>
		...
		<tr>
			<td width="182" valign="top">
				<div class="agency" id="resizable" >
					...
					<div class="agencybtn">
						<ul>
						<%if(EstateActivator.getHttpSessionService().isInResourceMap("business_enterprise_classify")){ %>
							<li><a href="javascript:void(0)" onclick="fbStart('企业分类','<%=BaseAction.rootLocation%>/department.business/web/client/group_add.jsp',300,80);"><span class="icoadd"></span>企业分类</a></li>
						<%} %>
						</ul>
					</div>
					<%} %>
						...
						<table width="100%" height="25" border="0" cellpadding="0" cellspacing="0">
							<tr>
								...
								<td width="105">
									...
									<enum:select styleClass="data" name="type" checked="sessionScope.Park_type" defaultValue="${param.type }" type="com.wiiy.business.preferences.enums.CustomerTypeEnum"/>
									</search:choose>
								</td>
...
<map name="Map" id="Map">
	...
	<area shape="rect" coords="55,1,74,20" href="javascript:void(0)" onclick="fbSearch('高级搜索','<%=BaseAction.rootLocation%>/department.business/web/client/customer_search.jsp',550,400);" />
</map>
</body>
</html>