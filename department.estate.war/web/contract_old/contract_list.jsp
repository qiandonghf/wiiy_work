...
<script type="text/javascript">
	...
	function initList(){
		...
		$("#list").jqGrid({
			...
			colModel: [
				...
				{label:'供应商名称',width:100,name:'supplier.name',align:"center"}, 
				{label:'客户名称',width:100,name:'customer.name',align:"center"}, 
			...
			gridComplete: function(){
				...
				for(var i = 0 ; i < ids.length; i++){
					var id = ids[i];
					var content = "";
					<%Map<String, ResourceDto> resourceMap = EngineeringActivator.getHttpSessionService().getResourceMap();
					boolean add = EngineeringActivator.getHttpSessionService().isInResourceMap("engineering_contract_add");
					boolean edit = EngineeringActivator.getHttpSessionService().isInResourceMap("engineering_contract_view");
					boolean view = EngineeringActivator.getHttpSessionService().isInResourceMap("engineering_contract_edit");
					boolean del = EngineeringActivator.getHttpSessionService().isInResourceMap("engineering_contract_delete");%>
					<%if(view){%>
	...
	function editById(id){
		fbStart('编辑','<%=basePath%>contract!edit.action?id='+id,650,475);
	}
...
<div class="emailtop">
	<div class="leftemail">
		<ul>
			<%if(add){ %>
			<li onmouseover="this.className='libg'" onmouseout="this.className=''" onclick="fbStart('新建','<%=basePath %>web/contract/contract_add.jsp',650,475);"><span><img src="core/common/images/emailadd.gif"/></span>添加</li>
			<%} %>
			<%if(del){ %>
