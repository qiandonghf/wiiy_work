...
<script type="text/javascript">
	...
	function initList(){
		...
		$("#list").jqGrid({
	    	url:'<%=basePath%>complaint!list.action?status=${param.status}',
			colModel: [
			...
			gridComplete: function(){
				...
				for(var i = 0 ; i < ids.length; i++){
					...
					<%}%>
					<%if(EstateActivator.getHttpSessionService().isInResourceMap("estate_base_complaint_list_edit")){%>
					content += "<img src=\"core/common/images/edit.gif\" width=\"14\" height=\"14\" title=\"编辑\" onclick=\"editById('"+id+"');\"  /> "; 
					<%}%>
					<%if(EstateActivator.getHttpSessionService().isInResourceMap("estate_base_complaint_list_handle")){%>
					content += "<img src=\"core/common/images/cl.png\" width=\"14\" height=\"14\" title=\"处理\" onclick=\"handle('"+id+"');\"  /> "; 
					<%}%>
					<%if(EstateActivator.getHttpSessionService().isInResourceMap("estate_base_complaint_list_delete")){%>
...
<div class="emailtop">
			<div class="leftemail">
				...
				<%if(EstateActivator.getHttpSessionService().isInResourceMap("estate_base_complaint_add")){%>
					<li onmouseover="this.className='libg'" onmouseout="this.className=''" onclick="fbStart('新建投诉单','<%=basePath %>web/property/complaint_add.jsp',600,222);"><span><img src="core/common/images/emailadd.gif" width="15" height="13" /></span>新建</li>
				<%} %>
				<%if(EstateActivator.getHttpSessionService().isInResourceMap("estate_base_complaint_list_delete")){%>
