...
<script type="text/javascript">
	...
	function initList(){
		...
		$("#list").jqGrid({
	    	url:'<%=basePath%>propertyFix!list.action?status=${param.status}',
			colModel: [
			...
			gridComplete: function(){
				...
				for(var i = 0 ; i < ids.length; i++){
					...
					var status=$(this).getCell(id,'status.title');
					if(status=='完成'||status=='挂起'){
					content += "<img src=\"core/common/images/viewbtn.gif\" width=\"14\" height=\"14\" title=\"查看\" onclick=\"viewById('"+id+"');\"  /> "; 
					}else{
					content += "<img src=\"core/common/images/viewbtn.gif\" width=\"14\" height=\"14\" title=\"查看\" onclick=\"viewById2('"+id+"');\"  /> "; 
					}
					<%}%>
					...
					content += "<img src=\"core/common/images/edit.gif\" width=\"14\" height=\"14\" title=\"编辑\" onclick=\"editById2('"+id+"');\"  /> "; 
					}
					<%}%>
	...
	function viewById(id){
		fbStart('查看物业报修单','<%=basePath %>propertyFix!view.action?id='+id,800,406);
	}
	function viewById2(id){
		fbStart('查看物业报修单','<%=basePath %>propertyFix!view.action?id='+id,800,197);
	}
	
	function editById(id){
		fbStart('编辑物业报修单','<%=basePath %>propertyFix!edit.action?id='+id,800,406);
	}
...
<div class="emailtop">
			<div class="leftemail">
				...
				<%if(EstateActivator.getHttpSessionService().isInResourceMap("estate_base_repair_add")){ %>
					<li onmouseover="this.className='libg'" onmouseout="this.className=''" onclick="fbStart('新建物业报修单','<%=basePath %>propertyFix!add.action',800,197);"><span><img src="core/common/images/emailadd.gif" width="15" height="13" /></span>新建</li>
				<%} %>
				<%if(EstateActivator.getHttpSessionService().isInResourceMap("estate_base_repair_hagin_edit")){ %>
