<%@page import="com.wiiy.estate.preferences.enums.ContractItemEnum"%>
...
<script type="text/javascript">
	...
	function initList(){
		...
		$("#list").jqGrid({
	    	url:'<%=BaseAction.rootLocation%>/department.estate/contract!estateContractList.action?type=${param.type}',
			colModel: [
				...
				{label:'合同企业', name:'customerName',index:'customer', align:"center"}, 
			...
			gridComplete: function(){
				...
				for(var i = 0 ; i < ids.length; i++){
					var id = ids[i];
					var content = "";
				<%if(EstateActivator.getHttpSessionService().isInResourceMap("estate_contract_view")){ %>
					content += "<img src=\"core/common/images/viewbtn.gif\" width=\"14\" height=\"14\" title=\"查看\" onclick=\"viewById('"+id+"');\"  /> "; 
				<%}%>
			if($(this).getCell(id,"state.title")!='已关闭'){
				<%if(EstateActivator.getHttpSessionService().isInResourceMap("estate_contractReview_add")){ %>
				...
				<%if(EstateActivator.getHttpSessionService().isInResourceMap("estate_contractExpireApproval_add")){ %>
					content += "<img src=\"core/common/images/application_add.png\" width=\"14\" height=\"14\" title=\"发起到期合同审批会签\" onclick=\"expire('"+id+"');\"  /> "; 
				<%}%>
				}
				if($(this).getCell(id,"state.title")!='已执行'){
				<%if(EstateActivator.getHttpSessionService().isInResourceMap("estate_contract_edit")){ %>
	...
	function editById(id){
		fbStart('编辑合同','<%=BaseAction.rootLocation%>/department.estate/contract!edit.action?id='+id,750,503);
	}
...
<body>
<form action="<%=BaseAction.rootLocation%>/department.estate/contract!export.action" id="exportForm" method="post">
...
<div class="emailtop" style="position:relative;z-index:99999;">
	<div class="leftemail">
		<ul>
		<%if(EstateActivator.getHttpSessionService().isInResourceMap("estate_contract_addCustomer")){ %>
			<li onmouseover="this.className='libg'" onmouseout="this.className=''" onclick="$('#meterlayer').toggle();" style="position:relative; z-index:99999;"><span><img src="core/common/images/emailadd.gif" width="15" height="13" /></span>新建<em class="em"><img src="core/common/images/arrow.png" /></em>					
				<div class="meterlayer" id="meterlayer" style="display:none; position:absolute; left:5px;" >
					<dl>
						<dt><a href="javascript:void(0)" onclick="fbStart('<%=ContractItemEnum.WYHT.getTitle() %>','<%=basePath %>web/contract/contract_add.jsp?type=<%=ContractItemEnum.WYHT.name() %>',750,493);"><%=ContractItemEnum.WYHT.getTitle() %></a></dt>
					</dl>
				</div>
			</li>
			<%} %>
