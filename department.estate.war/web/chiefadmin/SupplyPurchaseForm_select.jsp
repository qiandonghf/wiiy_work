...
<script type="text/javascript">
		...
		$("#browser").tree({
			...
			onClick: function(node){
				if($(this).tree("isLeaf",node.target)){
					...
					if(formId==null){
						$("#list").setGridParam({url:'<%=basePath%>supplyPurchaseForm!list.action',postData:{filters:''}}).trigger('reloadGrid'); 
					}else{
	...
	function initList(){
		...
		$("#list").jqGrid({
			url:'<%=basePath%>supplyPurchaseForm!list.action',
			datatype: 'json',
			mtype: 'POST',
			...
			colModel: [
				...
				{label:'申请表名称', name:'name', index:'name', align:"center"},
				{label:'申请人', name:'requisitioner.realName', align:"center"},
				{label:'申请日期', name:'applyTime', index:'applyTime', align:"center",formatter:'date',formatoptions:{srcformat:'Y-m-d',newformat:'Y-m-d'}},
			],
			height: height,
			width:width,
	...
	function submitSelectedSupply() {
		try{
			if(typeof(getOpener().eval("setSelectedForm")) == "function"){
				var ids = $("#list").jqGrid("getDataIDs");
				$(ids).each(function (index, id) {
					if ($("#list").find('#'+id+' input').prop('checked')) {
						var rowData = $("#list").jqGrid("getRowData", id);
						getOpener().setSelectedForm(rowData);
						return;
					}
				});
			} 
