...
import com.wiiy.activiti.system.DataDictInit;
...
public class Activator extends AbstractActivator {
	...
	public void startBundle(BundleContext context) throws Exception {
		...
		logger.debug("bundle 【activiti.war】 start");
	}
	...
	protected void registryFK(){
		...
		if (fkServiceRef != null) {
			...
			fkService.registryFK(getFKDescription(bundleContext,"WEB-INF/classes/com/wiiy/activiti/entity/"));
		} else {
	...
	public void stopBundle(BundleContext context) throws Exception {
		logger.debug("bundle 【activiti.war】 stop");
	}
	...
	public static UserPubService getUserPubService(){
		return getService(bundleContext, UserPubService.class);
	}
	...
	protected void initDataDict() {
		...
		if (serviceRef != null) {
			logger.info("###################################################################################bundle 【activiti.war】 initDataDict1");
				...
				public void serviceChanged(ServiceEvent serviceEvent) {
					if (serviceEvent.getType() == ServiceEvent.REGISTERED){
						...
						if (registeredService instanceof DataDictInitService) {
							logger.info("###################################################################################bundle 【activiti.war】 initDataDict2");
							DataDictInitService dictInitService = getService(bundleContext, DataDictInitService.class);
							dictInitService.init(new DataDictInit().getList());
						}
					}
