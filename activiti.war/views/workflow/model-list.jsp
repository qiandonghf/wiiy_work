...
<body>
		...
		<tbody>
			<c:forEach items="${list }" var="model">
				<tr>
					...
					<td>
						<a href="/activiti/service/editor?id=${model.id}" target="_blank">编辑</a>
						<a href="/activiti/workflow/model/deploy/${model.id}">部署</a>
						<a href="/activiti/workflow/model/export/${model.id}" target="_blank">导出</a>
                        <a href="/activiti/workflow/model/delete/${model.id}">删除</a>
					</td>
				</tr>
			</c:forEach>
		</tbody>
	...
	<div id="createModelTemplate" title="创建模型" class="template">
        <form id="modelForm" action="/activiti/workflow/model/create" target="_blank" method="post">
		<table>
			<tr>
				<td>名称：</td>
				<td>
