...
<head>
	...
	<script src="/activiti/js/module/activiti/workflow.js" type="text/javascript"></script>
		...
		<tbody>
			<c:forEach items="${page.result }" var="leave">
				...
				<tr id="${leave.id }" tid="${task.id }">
					...
					<td>
						<a class="trace" href='/activiti/workflow!resourceRead.action?processDefinitionId=${hpi.processDefinitionId}&resourceType=image' target="_blank" title="点击查看流程图">流程图</a>
					</td>
					<td>
						<c:forEach items="${leave.historicTaskInstanceList }" var="task">
							[name=${task.name},taskDefinitionKey=${task.taskDefinitionKey},<!-- processInstanceId=${task.processInstanceId},processDefinitionId=${task.processDefinitionId}, -->assignee=${task.assignee},claimTime=${task.claimTime},startTime=${task.startTime},endTime=${task.endTime}]<br>
