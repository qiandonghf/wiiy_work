...
<script type="text/javascript">
	...
	function initList(){
		...
		$("#list").jqGrid({
	    	url:'<%=basePath%>registrationBook!list.action',
	    	colModel: [
				{label:'客户名称',width:50,name:'customer.name',align:"center"},
   				{label:'置业顾问',width:50,name:'user.realName',align:"center", sortable:false}, 
   				{label:'到期是否可销售',width:50,name:'whetherSales.title',align:"center", hidden:true}, 
   				{label:'预定金额', name:'reservationAmount',width:30, align:"center"},
   			    {label:'工作电话', name:'phone', align:"center",width:50},
   				{label:'业务编号',width:50,name:'businessCode',align:"center",hidden:true}, 
   				{label:'预订编号',width:50,name:'bookCode',align:"center",hidden:true}, 
   				{label:'房屋名称',width:30,name:'roomName',align:"center"}, 
   				{label:'房屋编号',width:50,name:'roomCode',align:"center",hidden:true},
   				{label:'身份证',name:'idCard', align:"center",width:50,hidden:true}, 
			    ...
			    {label:'邮政编号', name:'zipCode', align:"center", hidden:true},
			    {label:'通讯地址', name:'adress', align:"center", hidden:true},
			    ...
			    {label:'状态', width:'30' ,name:'entityStatus.title', align:"center",formatter:checkStatus}, 
			    {label:'出租/出售', width:'30' ,name:'rentStatus.title', align:"center"},
	...
	function viewById(id){
		fbStart('查看','<%=basePath%>registrationBook!view.action?id='+id,620,475);
	}	
	function editById(id){
		fbStart('编辑','<%=basePath%>registrationBook!edit.action?id='+id,620,415);
	}
	...
	function changeState(id){
		$.post("<%=basePath%>registrationBook!changeState.action?id="+id,function(data){
	...
	function deleteById(id){
		if(confirm("确定删除")){
			$.post("<%=basePath%>registrationBook!delete.action?id="+id,function(data){
				showTip(data.result.msg,2000);
	        	if(data.result.success){
	        		reloadList();
	        	}
	...
	function deleteSelected(){
		...
		if(ids==""){
			...
			$.post("<%=basePath%>registrationBook!delete.action?ids="+ids,function(data){
				showTip(data.result.msg,2000);
	        	if(data.result.success){
	        		reloadList();
	        	}
	...
	function add(){
		fbStart('新建','<%=basePath %>web/registration/registration_add.jsp',620,415);
	}
	function doSearch(){
