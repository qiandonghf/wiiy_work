...
<title>无标题文档</title>
	...
	<script type="text/javascript">
		...
		function initChange(){
			var audit = $("input[name='sale.audit']");
			var finished = $("input[name='sale.finished']");
			var published = $("input[name='sale.published']");
			if($(audit).val() == 'YES'){
		...
		function generateCode(){
			...
			$.post("<%=basePath%>sale!generateCode.action",function(data){
				if(data.result.success){
					showTip("生成编号成功",2000);
...
<input name="sale.id" value="${result.value.id }" type="hidden" />
<div class="basediv">
<div class="divlays" style="margin:0px;">
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
		<tr>
       		<td class="layertdleft100"><span class="psred">*</span>项目名称：</td>
			<td class="layerright"><input name="sale.name" type="text" class="inputauto" value="${result.value.name }" /></td>
       		<td class="layertdleft100"><span class="psred">*</span>编号：</td>
			<td class="layerright">
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
						<td width="100"><input id="code" name="sale.code" value="${result.value.code }" type="text" class="inputauto" /></td>
       	...
       	<tr>
		    ...
		    <td class="layerright"><input name="sale.abbreviation" type="text" class="inputauto" value="${result.value.abbreviation}" /></td>
       		<td class="layertdleft100"><span class="psred">*</span>项目进度数：</td>
		    <td class="layerright"><input name="sale.schedules" onkeyup="checkDouble(this);" value="${result.value.schedules}" class="inputauto"/>
		    </td>
       	</tr>
       	<tr>
			...
			<td class="layerright">
				<table width="143" border="0" cellspacing="0" cellpadding="0">
			        <tr>
			          <td>
			          	<input id="supplierId" name="sale.supplierId" value="${result.value.supplierId }" type="hidden"/>
			          	<input id="supplier" name="sale.supplierName" class="inputauto" value="${result.value.supplierName }" onclick="selectCustomer('supplier')" readonly="readonly"/></td>
			...
			<td class="layerright">
				<table width="143" border="0" cellspacing="0" cellpadding="0">
			        <tr>
			          <td>
				          <input id="customerId" name="sale.customerId" value="${result.value.customerId }" type="hidden"/>
				          <input id="customer" name="sale.customerName" class="inputauto" value="${result.value.customerName }" onclick="selectCustomer('customer')" readonly="readonly"/>
				      </td>
       	...
       	<tr>
			...
			<td class="layerright" >
				<enum:select name="sale.status" checked="result.value.status" type="com.wiiy.common.preferences.enums.ProjectStatusEnum" />
		    </td>
		    <td class="layertdleft100">重要程度：</td>
			<td class="layerright">
				<enum:select name="sale.priority" checked="result.value.priority" type="com.wiiy.common.preferences.enums.ProjectPriorityEnum" />
		    </td>
    	</tr>
    	<tr>
			...
			<td class="layerright">
				<table border="0" cellspacing="0" cellpadding="0">
			        <tr>
			          <td>
			          	<input id="managerId" name="sale.managerId" value="${result.value.manager.id }" type="hidden" />
			...
			<td class="layerright" >
				<table border="0" cellspacing="0" cellpadding="0">
			        <tr>
			          <td>
			          	<input id="handlingId" name="sale.handlingId" value="${result.value.handling.id }" type="hidden" />
       	...
       	<tr>
       		...
       		<td class="layerright" >
	       		<dd:select name="sale.currencyTypeId" checked="result.value.currencyTypeId" key="project.0001"/>
       		</td>
       		<td class="layertdleft100">金额：</td>
	      	<td class="layerright"><input name="sale.amount" value='<fmt:formatNumber value="${result.value.amount }" pattern="0.00" />' type="text" value="0.00" class="inputauto"  /></td>
       	</tr>
       	<tr>
    		<td class="layertdleft100">预计花费：</td>
	      	<td class="layerright"><input name="sale.expectedCost" value='<fmt:formatNumber value="${result.value.expectedCost }" pattern="0.00" />' type="text" value="0.00" class="inputauto"/></td>
    		<td class="layertdleft100">实际花费：</td>
    		<td class="layerright" ><input name="sale.actualCost" value='<fmt:formatNumber value="${result.value.actualCost }" pattern="0.00" />' type="text" value="0.00" class="inputauto"/></td>
    	</tr>
    	<tr>
    		<td class="layertdleft100">收付方式：</td>
       		<td class="layerright">
				<enum:select name="sale.payment" checked="result.value.payment" type="com.wiiy.common.preferences.enums.PaymentTypeEnum"/>
       		</td>
       		<td class="layertdleft100">结算方式：</td>
       		<td class="layerright" >
				<enum:select name="sale.settlement" checked="result.value.settlement" type="com.wiiy.common.preferences.enums.SettlementTypeEnum" />
       		</td>
    	</tr>
    	<tr>
     		...
     		<td class="layerright">
	     		<table border="0" cellspacing="0" cellpadding="0">
			        <tr>
			          <td><input id="startDate" name="sale.startDate" 
		    ...
		    <td class="layertdleft100"><span class="psred">*</span>项目结束日期：</td>
     		<td class="layerright">
	     		<table border="0" cellspacing="0" cellpadding="0">
			        <tr>
			          <td><input id="endDate" name="sale.endDate"
    	...
    	<tr>
    		<td class="layertdleft100"><span class="psred">*</span>项目签订日期：</td>
	      	<td class="layerright">
		      	<table border="0" cellspacing="0" cellpadding="0">
		          <tr>
		            <td><input id="signDate" name="sale.signDate" readonly="readonly"
	      	...
	      	<td class="layerright" colspan="2">
				<label><input name="sale.audit" type="checkbox" onclick="changeState(this);" value="${result.value.audit }" /><span>未审核</span></label>
				<label><input name="sale.finished" type="checkbox" onclick="changeState(this);" value="${result.value.finished }" /><span>未完成</span></label>
		   		<label><input name="sale.published" type="checkbox" onclick="changeState(this);" value="${result.value.published }" />公开标志</label>
	      	</td>
       	</tr>
       	<tr>
	    	<td class="layertdleft100">项目简介：</td>
	    	<td colspan="3" class="layerright" style="padding-bottom:3px;">
	    		<textarea name="sale.introduction" class="inputauto" style="resize:none;height:40px;">${result.value.introduction }</textarea>
	    	</td>
	    </tr>
       	<tr>
       		<td class="layertdleft100">备注信息：</td>
			<td class="layerright"  colspan="3">
	    		<textarea name="sale.memo" class="inputauto" style="resize:none;height:40px;">${result.value.memo }</textarea>
			</td>
       	</tr>
  </table>
  </div>
