...
<script type="text/javascript">
 $(document).ready(function(){
	...
	boolean dayReport = SynthesisActivator.getHttpSessionService().isInResourceMap("synthesis_personal_viewDayReportCollectList");
	boolean weekReport = SynthesisActivator.getHttpSessionService().isInResourceMap("synthesis_personal_viewWeekReportList");
	boolean monthReport = SynthesisActivator.getHttpSessionService().isInResourceMap("synthesis_personal_viewMonthReportList");
	%>
});
...
<div class="emailtop">
	<div class="leftemail">
		<ul>
			...
			<%} %>
			<%if(weekReport){%>
			<li style="background-color:#D2E6FF" onmouseover="this.className='libg'" onmouseout="this.className=''"  onclick="location.href='<%=basePath %>workReport!employeeCountList.action'"><span><img src="core/common/images/weeklycount.png" /></span>周报</li>
			<%} %>
			<%if(monthReport){%>
			<li onmouseover="this.className='libg'" onmouseout="this.className=''" 	onclick="location.href='<%=basePath %>workReport!employeeMonthCountList.action'"><span><img src="core/common/images/monthcount.png" /></span>月报</li>
			<%} %>
      ...
      <tr>
    	<td colspan="13" align="center" class="workcountth">
    		<em style="position:relative; top:3px; padding-right:5px;">
    			<img src="core/common/images/dataviewleft.png" width="15" height="16" onclick="location.href='<%=basePath %>workReport!employeeCountList.action?myYear=<fmt:formatDate value="${prevYear}" pattern="yyyy-MM-dd HH:mm:ss"/>'"/>
    		</em>${year}年<em style="position:relative; top:3px; padding-left:5px;">
    			<img src="core/common/images/dataviewright.png" onclick="location.href='<%=basePath %>workReport!employeeCountList.action?myYear=<fmt:formatDate value="${nextYear}" pattern="yyyy-MM-dd HH:mm:ss"/>'"/>
    		</em>
    	</td>
      </tr>
    <tr>
