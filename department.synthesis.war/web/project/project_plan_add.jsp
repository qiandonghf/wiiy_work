...
<title>无标题文档</title>
	...
	<script type="text/javascript">
		...
		function changeState(obj){
			...
			if($(obj).attr("name") == 'plan.finished'){
				var p = $(obj).parent();
				var c = $(p).find("span");
				if($(obj).val() == 'NO')$(c).text("未完成");
				else $(c).text("已完成");
			}else if($(obj).attr("name") == 'plan.audit'){
				var p = $(obj).parent();
				var c = $(p).find("span");
		...
		function selectUser(id){
			var title = '选择';
			clicked = id;
			fbStart("经手人",'<%=BaseAction.rootLocation %>/core/user!select.action',520,400);
		} 
...
<body>
<form action="<%=basePath %>plan!save.action" method="post" name="form1" id="form1">
<input type="hidden" name="plan.projectId" value="${param.id }"/>
<div class="basediv">
		...
		<tr>
			...
			<td class="layerright">
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
						<td width="100"><input id="code" name="plan.code" type="text" class="inputauto" /></td>
     		...
     		<td class="layerright">
	     		<table border="0" cellspacing="0" cellpadding="0">
			        <tr>
			          <td><input id="time" name="plan.time" readonly="readonly" class="inputauto" onclick="showCalendar('time');"/></td>
       	...
       	<tr>
			...
			<td class="layerright">
				<input name="plan.schedule" onkeyup="checkDouble(this);" class="inputauto"  />
			</td>
       	...
       	<tr>
			...
			<td class="layerright">
				<table border="0" cellspacing="0" cellpadding="0">
			        <tr>
			          <td>
			          	<input id="handlingId" name="plan.handlingId" type="hidden" />
	      	...
	      	<td class="layerright" colspan="2">
				<label><input name="plan.audit" type="checkbox" onclick="changeState(this);" value="NO" /><span>未审核</span></label>
				<label><input name="plan.finished" type="checkbox" onclick="changeState(this);" value="NO" /><span>未完成</span></label>
       	...
       	<tr>
			...
			<td class="layerright" colspan="3">
	    		<textarea name="plan.memo" class="inputauto" style="resize:none;height:80px;"></textarea>
			</td>
       	</tr>
  </table>
  </div>
