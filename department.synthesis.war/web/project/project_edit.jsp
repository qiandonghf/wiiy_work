...
<title>无标题文档</title>
	...
	<script type="text/javascript">
		...
		function initChange(){
			var audit = $("input[name='synthesis.audit']");
			var finished = $("input[name='synthesis.finished']");
			var published = $("input[name='synthesis.published']");
			if($(audit).val() == 'YES'){
		...
		function initForm(){
			$("#form1").validate({
				rules: {
					"synthesis.name":"required",
					"synthesis.code":"required",
					"synthesis.startDate":"required",
					"synthesis.endDate":"required",
					"synthesis.signDate":"required"
				},
				messages: {
					"synthesis.name":"请输入项目名称",
					"synthesis.code":"请输入项目编号",
					"synthesis.startDate":"请选择有效日期开始时间",
					"synthesis.endDate":"请选择有效日期结束时间",
					"synthesis.signDate":"请选择签订日期"
				},
		...
		function generateCode(){
			...
			$.post("<%=basePath%>synthesis!generateCode.action",function(data){
				if(data.result.success){
					showTip("生成编号成功",2000);
...
<input name="synthesis.id" value="${result.value.id }" type="hidden" />
<div class="basediv">
<div class="divlays" style="margin:0px;">
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
		<tr>
       		<td class="layertdleft100"><span class="psred">*</span>项目名称：</td>
			<td class="layerright"><input name="synthesis.name" type="text" class="inputauto" value="${result.value.name }" /></td>
       		<td class="layertdleft100"><span class="psred">*</span>编号：</td>
			<td class="layerright">
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
						<td width="100"><input id="code" name="synthesis.code" value="${result.value.code }" type="text" class="inputauto" /></td>
       	...
       	<tr>
		    ...
		    <td class="layerright"><input name="synthesis.abbreviation" type="text" class="inputauto" value="${result.value.abbreviation}" /></td>
       		<td class="layertdleft100"><span class="psred">*</span>项目进度数：</td>
		    <td class="layerright"><input name="synthesis.schedules" onkeyup="checkDouble(this);" value="${result.value.schedules}" class="inputauto"/>
		    </td>
       	</tr>
       	<tr>
			...
			<td class="layerright">
				<table width="143" border="0" cellspacing="0" cellpadding="0">
			        <tr>
			          <td>
			          	<input id="supplierId" name="synthesis.supplierId" value="${result.value.supplierId }" type="hidden"/>
			          	<input id="supplier" name="synthesis.supplierName" class="inputauto" value="${result.value.supplierName }" onclick="selectCustomer('supplier')" readonly="readonly"/></td>
			...
			<td class="layerright">
				<table width="143" border="0" cellspacing="0" cellpadding="0">
			        <tr>
			          <td>
				          <input id="customerId" name="synthesis.customerId" value="${result.value.customerId }" type="hidden"/>
				          <input id="customer" name="synthesis.customerName" class="inputauto" value="${result.value.customerName }" onclick="selectCustomer('customer')" readonly="readonly"/>
				      </td>
       	...
       	<tr>
			...
			<td class="layerright" >
				<enum:select name="synthesis.status" checked="result.value.status" type="com.wiiy.common.preferences.enums.ProjectStatusEnum" />
		    </td>
		    <td class="layertdleft100">重要程度：</td>
			<td class="layerright">
				<enum:select name="synthesis.priority" checked="result.value.priority" type="com.wiiy.common.preferences.enums.ProjectPriorityEnum" />
		    </td>
    	</tr>
    	<tr>
			...
			<td class="layerright">
				<table border="0" cellspacing="0" cellpadding="0">
			        <tr>
			          <td>
			          	<input id="managerId" name="synthesis.managerId" value="${result.value.manager.id }" type="hidden" />
			...
			<td class="layerright" >
				<table border="0" cellspacing="0" cellpadding="0">
			        <tr>
			          <td>
			          	<input id="handlingId" name="synthesis.handlingId" value="${result.value.handling.id }" type="hidden" />
       	...
       	<tr>
       		...
       		<td class="layerright" >
	       		<dd:select name="synthesis.currencyTypeId" checked="result.value.currencyTypeId" key="project.0001"/>
       		</td>
       		<td class="layertdleft100">金额：</td>
	      	<td class="layerright"><input name="synthesis.amount" value='<fmt:formatNumber value="${result.value.amount }" pattern="0.00" />' type="text" value="0.00" class="inputauto"  /></td>
       	</tr>
       	<tr>
    		<td class="layertdleft100">预计花费：</td>
	      	<td class="layerright"><input name="synthesis.expectedCost" value='<fmt:formatNumber value="${result.value.expectedCost }" pattern="0.00" />' type="text" value="0.00" class="inputauto"/></td>
    		<td class="layertdleft100">实际花费：</td>
    		<td class="layerright" ><input name="synthesis.actualCost" value='<fmt:formatNumber value="${result.value.actualCost }" pattern="0.00" />' type="text" value="0.00" class="inputauto"/></td>
    	</tr>
    	<tr>
    		<td class="layertdleft100">收付方式：</td>
       		<td class="layerright">
				<enum:select name="synthesis.payment" checked="result.value.payment" type="com.wiiy.common.preferences.enums.PaymentTypeEnum"/>
       		</td>
       		<td class="layertdleft100">结算方式：</td>
       		<td class="layerright" >
				<enum:select name="synthesis.settlement" checked="result.value.settlement" type="com.wiiy.common.preferences.enums.SettlementTypeEnum" />
       		</td>
    	</tr>
    	<tr>
     		...
     		<td class="layerright">
	     		<table border="0" cellspacing="0" cellpadding="0">
			        <tr>
			          <td><input id="startDate" name="synthesis.startDate" 
		    ...
		    <td class="layertdleft100"><span class="psred">*</span>项目结束日期：</td>
     		<td class="layerright">
	     		<table border="0" cellspacing="0" cellpadding="0">
			        <tr>
			          <td><input id="endDate" name="synthesis.endDate"
    	...
    	<tr>
    		<td class="layertdleft100"><span class="psred">*</span>项目签订日期：</td>
	      	<td class="layerright">
		      	<table border="0" cellspacing="0" cellpadding="0">
		          <tr>
		            <td><input id="signDate" name="synthesis.signDate" readonly="readonly"
	      	...
	      	<td class="layerright" colspan="2">
				<label><input name="synthesis.audit" type="checkbox" onclick="changeState(this);" value="${result.value.audit }" /><span>未审核</span></label>
				<label><input name="synthesis.finished" type="checkbox" onclick="changeState(this);" value="${result.value.finished }" /><span>未完成</span></label>
		   		<label><input name="synthesis.published" type="checkbox" onclick="changeState(this);" value="${result.value.published }" />公开标志</label>
	      	</td>
       	</tr>
       	<tr>
	    	<td class="layertdleft100">项目简介：</td>
	    	<td colspan="3" class="layerright" style="padding-bottom:3px;">
	    		<textarea name="synthesis.introduction" class="inputauto" style="resize:none;height:40px;">${result.value.introduction }</textarea>
	    	</td>
	    </tr>
       	<tr>
       		<td class="layertdleft100">备注信息：</td>
			<td class="layerright"  colspan="3">
	    		<textarea name="synthesis.memo" class="inputauto" style="resize:none;height:40px;">${result.value.memo }</textarea>
			</td>
       	</tr>
  </table>
  </div>
