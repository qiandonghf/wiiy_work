...
<script type="text/javascript">
	...
	function setSelectedCars(cars){
		var ids = "";
		var licenseNos = "";
		for(var i = 0; i < cars.length; i++){
			ids += ","+cars[i].id;
			licenseNos += cars[i].licenseNo+",";
		}
		ids = deleteLastCharWhenMatching(ids,",");
		licenseNos = deleteLastCharWhenMatching(licenseNos,",");
		$("#carIds").val(ids);
		$("#carLicenseNo").val(licenseNos);
	}
	function selectCars(){
		var ids = $("#carIds").val();
		fbStart('车辆选择','<%=basePath%>car!select.action?ids='+ids,400,390);
	}
    ...
    <tr>
        ...
        <tr>
          <td width="146">
            ...
            <input id="carLicenseNo" name="carApply.licenseNo" value="${result.value.licenseNo}" type="text" class="inputauto" onclick="selectCars();"/>
             <input id="carIds" name="carApply.carIds" type="hidden" value="${result.value.carIds}"/>
          </td>
          <td><img src="core/common/images/outdiv.gif" width="20" height="22" onclick="selectCars();"/></td>
		  <td></td>
        </tr>
      </table></td>
       </tr>
