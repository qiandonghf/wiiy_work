...
public class CarAnnualJob extends RepeatJob{
	
	protected ApplicationContext applicationContext;
	private CarService carService;
	private List<Car> cars = new ArrayList<Car>();
	
	public CarAnnualJob(ApplicationContext applicationContext) {
	...
	protected void execute() {
		...
		cars = carService.getListByFilter(new Filter(Car.class).le("annualDate",remindTime.getTime())).getValue();
	}
	...
	class CarArrears extends TimerTask{
		...
		public void run() {
			if(cars!=null&&cars.size()>0){
				...
				String content = "尊敬的"+user.getRealName()+"；车牌号为【"+licenseNo+"】的车辆年检即将到期， 请尽快处理";
				String receiverMobile = "";
				if(user.getMobile()!=null){
					receiverMobile = user.getMobile();
				}
				String subject = "车辆年检到期提醒";
