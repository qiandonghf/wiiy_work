...
<script type="text/javascript">
	...
	function initList(){
		var height = $("#resizable").height()-45;
		var width = $("#resizable").width();
		$("#list").jqGrid({
			url:'<%=basePath%>billPlanRent!list.action?feeType=${feeType}',
			colModel: [
			...
			gridComplete: function(){
				...
				for(var i = 0 ; i < ids.length; i++){
					var id = ids[i];
					var content = "";
					<%if(ProjectActivator.getHttpSessionService().isInResourceMap("common_billPlanRent_view")){%>
					content += "<img src=\"core/common/images/viewbtn.gif\" width=\"14\" height=\"14\" title=\"查看\" onclick=\"viewById('"+id+"');\"  /> "; 
					<%}%>
					<%if(ProjectActivator.getHttpSessionService().isInResourceMap("common_billPlanRent_checkin")){%>
					content += "<img src=\"core/common/images/rece2.gif\" width=\"14\" height=\"14\" title=\"收款出账\" onclick=\"checkinById('"+id+"');\"  /> "; 
					<%}%>
					<%if(ProjectActivator.getHttpSessionService().isInResourceMap("common_billPlanRent_checkout")){%>
...
<body>
	...
	<search:choose dataType="com.wiiy.common.preferences.enums.BillPlanStatusEnum" field="status" op="eq">
									...
									<tbody>
										<tr>
											...
											<td width="90" style="padding-left: 3px;padding-right: 3px;">
												<search:choose dataType="com.wiiy.common.preferences.enums.BusinessFeeEnum" field="feeType" op="eq">
												<enum:select type="com.wiiy.common.preferences.enums.BusinessFeeEnum" styleClass="data"/>
												</search:choose>
											</td>
