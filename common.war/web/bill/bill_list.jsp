...
<script>
		...
		$("#list").jqGrid({
			...
			gridComplete: function(){
				...
				for(var i = 0 ; i < ids.length; i++){
					...
					content += "<a href=\"javascript:void(0)\" onclick=\"invoiceById('"+id+"');\">开票</a> ";
	...
	//联合开票
	function invoiceByIds(){
		var ids = $("#list").jqGrid("getGridParam","selarrrow");
		if(ids!=''){
			var allIds = $("#list").jqGrid('getDataIDs');
			var newIds = ","+ids+",";
			var customerId = "";
			for(var i = 0 ; i < allIds.length; i++){
				var id = allIds[i];
				var newId = ","+id+",";
				if(newIds.indexOf(newId)>-1){
					var newCustomerId = $("#list").getCell(id,"customerId");
					if(customerId!=""&&customerId!=newCustomerId){
						showTip("请选择相同企业",2000);
						return;
					}
			...
			if(confirm("确定要开票吗")){
				fbStart('开票','<%=basePath %>bill!invoiceById.action?ids='+ids,500,250);
			}
		}else{
			showTip("请选择账单",2000);
		}
			...
			<tr>
				...
				<td width="100%" valign="top">
					<div class="emailtop">
						<div class="leftemail">
							<ul>
							<%if(ProjectActivator.getHttpSessionService().isInResourceMap("settle_cost_collect_billing_cost")){ %>
								<li onmouseover="this.className='libg'" onmouseout="this.className=''" onclick="fbStart('缴费通知单','<%=basePath%>bill!inInformList.action',800,610);"><span><img src="core/common/images/bill_03.gif"/></span>缴费通知单</li>
							<%} %>
							<%if(ProjectActivator.getHttpSessionService().isInResourceMap("settle_cost_collect_billing_unit")){ %>
								<li onmouseover="this.className='libg'" onmouseout="this.className=''" onclick="invoiceByIds()"><span><img src="core/common/images/bill_03.gif"/></span>联合开票</li>
							<%} %>
							</ul>
						</div>
					</div>
