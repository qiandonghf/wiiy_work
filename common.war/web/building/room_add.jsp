...
<script type="text/javascript">
	...
	function checkRange(id1,id2,text){
		...
		if ($("#totalRent").val()!="") {
			if(checkDouble("totalRent","总价")==false){
				return;
			}
		}
		if(Number($("#"+id2).attr("value"))<Number($("#"+id1).attr("value"))){
			return false;
		}else{
			return true;
		}
...
<div class="divlays">
	<table id="mainTable" width="100%" border="0" cellspacing="0" cellpadding="0">
    	<tr>
      		...
      		<td class="layerright">${building.park.name}</td>
			<td class="layertdleft100">所在楼宇：</td>
			<td class="layerright" name="building">${building.name}</td>
      		<td class="layertdleft100">业务划分：</td>
	      	<td class="layerright">
	      		<select name="room.department">
	      			<option value="">----请选择----</option>
	      			<option value="<%=DepartmentEnum.SALE %>" <c:if test="${department eq 'SALE'}">selected="selected"</c:if>><%=DepartmentEnum.SALE.getTitle() %></option>
	      			<option value="<%=DepartmentEnum.BUSINESS %>" <c:if test="${department eq 'BUSINESS'}">selected="selected"</c:if>><%=DepartmentEnum.BUSINESS.getTitle() %></option>
	      		</select>
	      	</td>
	    </tr>
	    <tr>
	    ...
	    <tr>
			...
			<td class="layerright">
	      		<input id="heightId" name="room.height" type="text" class="input100 doubleformat"/>&nbsp;(米)
			</td>
