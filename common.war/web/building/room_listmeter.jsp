...
<script type="text/javascript">
	...
	function initList(list){
		$("#"+list).jqGrid({
	    	url:'<%=BaseAction.rootLocation %>/department.estate/roomMeterFee!list.action?id=${param.id}',
			datatype: 'json',
	...
	function editById(id){
		fbStart('编辑水电表费用','<%=BaseAction.rootLocation %>/department.estate/roomMeterFee!edit.action?id='+id,400,186);
	}
	
	function deleteById(id){
		if(confirm("您确定要删除")){
			$.post("<%=BaseAction.rootLocation %>/department.estate/roomMeterFee!delete.action?id="+id,function(data){
...
<body style="background:#fff;">
	<div class="pm_msglist">
			<div class="emailtop">
				<div class="leftemail">
					<ul>
						<li onmouseover="this.className='libg'" onmouseout="this.className=''" onclick="fbStart('水电表费用设置','<%=basePath %>web/building/room_listmeter_add.jsp?id=${param.id }',400,186);"><span><img src="core/common/images/emailadd.gif" width="15" height="13" /></span>水电表费用设置</li>
					</ul>
				</div>
			</div>
