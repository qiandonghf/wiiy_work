...
function initList(list){
	...
	$("#"+list).jqGrid({
    	url:'<%=basePath%>room!roomList.action?type=${param.type}&department=${param.department}',
		colModel: [
		...
		gridComplete: function(){
			...
			<%if(ProjectActivator.getHttpSessionService().isInResourceMap("pb_room_view")){%>
				content += "<img src=\"core/common/images/viewbtn.gif\" width=\"14\" height=\"14\" title=\"查看\" onclick=\"viewById('"+id+"','"+name+"');\"  /> ";
			<%}%>
...
function editById(id){
	fbStart('编辑房间信息','<%=basePath%>room!edit.action?id='+id,700,498);
}
function viewById(id,name){
	fbStart(name,'<%=basePath%>room!info.action?id='+id,800,526);
