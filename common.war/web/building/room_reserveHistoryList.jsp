...
<script type="text/javascript">
	...
	function initList(list){
		...
		boolean view = ProjectActivator.getHttpSessionService().isInResourceMap("park_residential_history_view");
		%>
		$("#"+list).jqGrid({
			url:'<%=BaseAction.rootLocation%>/department.sale/residential!list.action?id=${param.id}',
	    	colModel: [
	...
	function viewById(id){
		fbStart('查看','<%=BaseAction.rootLocation%>/department.sale/residential!view.action?id='+id,620,475);
	}
	...
	function changeState(id){
		$.post("<%=BaseAction.rootLocation%>/department.sale/residential!changeState.action?id="+id,function(data){
